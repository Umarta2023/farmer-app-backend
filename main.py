...
@app.get("/")
def read_root():
    return {"message": "API для фермеров, версия 2.0.0"}

# --- Эндпоинты без префикса ---
...
@app.get("/users/{user_id}/announcements", response_model=List[announcement_schema.AnnouncementDisplay], tags=["Users"])
...
def create_new_announcement(
    ...
    if image:
        file_path = f"uploads/{uuid.uuid4()}_{image.filename}"
        with open(file_path, "wb") as buffer:
            shutil.copyfileobj(image.file, buffer)
        image_url_to_save = f"/uploads/{uuid.uuid4()}_{image.filename}"
